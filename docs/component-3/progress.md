...
| Creation summary | Successful POST or authoritative reconciliation opens a dedicated **Reservation Created** summary populated only from the API DTO: reference, station, localized schedule, kWh, and exact server status. It contains no Approved/QR claim and explicitly explains that Pending is not Approved or QR-ready. Actions open details, a freshly API-loaded Pending list, or station search. |
...
| Lifecycle/errors | Loading, empty, error, and retry states use the shared state component. List/detail ViewModels retain the active server view, loaded data, useful scroll position, and stale-request generation across configuration recreation. Both screens silently refresh on return and every 30 seconds while visible, preserve existing content during transient refresh errors, and expose the error without deriving status from the phone clock. This also refreshes after create/update/cancel navigation and after later web approval/rejection. Booking ViewModels retain selected slot, quantity, idempotency key, baseline IDs, and in-flight/reconciliation state; back from review restores a valid draft and success clears it. |
...
- **Branch baseline:** this Android current/pending/detail update started from clean shared commit `95e4580`, where `feature/reservation-workflow` and `develop` matched. Member 1's user API/session implementation and Member 2's station/slot integration remain preserved.
...
| Git branch | PASS: implementation is on `feature/reservation-workflow`, starting from the shared `feature/reservation-workflow`/`develop` commit `95e4580`. |
...
| Automated tests | PASS: after this update, `clean testDebugUnitTest assembleDebug lintDebug` completed successfully in 45 seconds with 53 executed Gradle tasks, **7 passing local JVM tests**, a debug APK, and a generated lint report with no lint errors; a final incremental rerun also passed. The current backend suite is **39/39** and the current web check is **22/22**. |
...
Commit message requested for this Android list/detail update:

`feat(android): add current pending and reservation detail screens`
